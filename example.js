...
function createNewMemberButton(team) {
    ...
    btn.onclick = () => {
        const newMemberName = getValue(`name-input-${team.id}`);
        const newMemberPosition = getValue(`position-input-${team.id}`);
        const newMember = new Member(newMemberName, newMemberPosition);
        team.members. push(newMember);


           // new Member(getValue(`name-input-${team.id}`))
